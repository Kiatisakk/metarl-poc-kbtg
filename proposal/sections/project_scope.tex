...
\begin{description}[style=multiline,leftmargin=1.25cm,labelindent=0cm]
    \item[Problem Statement] First Jobbers must allocate limited surplus while future shortfall risk is uncertain.
    \item[Proposed Solution] A history-aware payday allocation recommendation with an explanation of the liquidity trade-off.
    \item[Target Users] Salaried 22--30-year-old users with a small buffer and one short-term goal; this is a validation hypothesis.
    \item[Value Proposition] Easier, more risk-aware allocation for users and an auditable Track 2 personalization experiment for K PLUS.
    \item[Track Perspective] MDP formulation, DP benchmark, Bayesian belief update, and a held-out Meta-RL test.
\end{description}
